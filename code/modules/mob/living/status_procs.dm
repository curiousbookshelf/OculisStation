...
/mob/living/proc/Sleeping(amount, is_voluntary = FALSE) //Can't go below remaining duration // NOVA EDIT: Enhanced sleep - ORIGINAL: /mob/living/proc/SetSleeping(amount) //Can't go below remaining duration
	...
	if(S)
		...
		S = apply_status_effect(/datum/status_effect/incapacitating/sleeping, amount, is_voluntary) // NOVA EDIT CHANGE - voluntary arg - Original: S = apply_status_effect(/datum/status_effect/incapacitating/sleeping, amount, is_voluntary) // NOVA EDIT: Enhanced sleep - ORIGINAL: S = apply_status_effect(/datum/status_effect/incapacitating/sleeping, amount)
	return S
...
/mob/living/proc/add_quirk(datum/quirk/quirktype, client/override_client, add_unique = TRUE, announce = TRUE, quirk_transfer = FALSE,) // OCULIS EDIT add quirk_transfer
	...
	if(quirk.add_to_holder(new_holder = src, client_source = override_client, unique = add_unique, announce = announce, quirk_transfer = quirk_transfer)) // OCULIS EDIT add quirk_transfer
		return TRUE
	qdel(quirk)
	return FALSE

...
/datum/computer_file/program/messenger/proc/quick_reply_prompt(mob/living/user, datum/pda_chat/chat)
	...
	send_message(user, input_message, list(chat)) // NOVA EDIT CHANGE - ORIGINAL: send_message(user, input_message, list(chat))
...
/datum/computer_file/program/messenger/proc/send_message(atom/source, message, list/targets, everyone = FALSE)
	...
	if(!send_message_signal(source, message, target_messengers, photo_asset_key, everyone, FALSE, null, null))
		return FALSE

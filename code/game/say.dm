 ...
 * - mute_bypass (bool): if we should ignore muteness for this check // OCULIS EDIT ADDITION
 *
 * Returns:
 * 	TRUE of FASE depending on if our movable can speak
 */
/atom/movable/proc/try_speak(message, ignore_spam = FALSE, forced = null, filterproof = FALSE, mute_bypass = FALSE) // OCULIS EDIT, ORIGINAL: /atom/movable/proc/try_speak(message, ignore_spam = FALSE, forced = null, filterproof = FALSE)
	return can_speak()
...
/atom/movable/proc/compose_message(atom/movable/speaker, datum/language/message_language, raw_message, radio_freq, radio_freq_name, radio_freq_color, list/spans, list/message_mods = list(), visible_name = FALSE)
	...
	var/realnamepart = "[speaker.get_voice(TRUE)]" // IRIS EDIT -- NTSL

	//End name span.
	//var/endspanpart = "</span>" // IRIS EDIT OLD -- NTSL
	var/endspanpart = "</span></a>" // IRIS EDIT NEW
	...
	// IRIS EDIT OLD -- NTSL down there
//	return "[spanpart1][spanpart2][freqpart][languageicon][compose_track_href(speaker, namepart)][namepart][compose_job(speaker, message_language, raw_message, radio_freq)][endspanpart][messagepart]"
	// IRIS EDIT NEW -- NTSL down there
...
/atom/movable/virtualspeaker/Initialize(mapload, atom/movable/M, _radio)
	...
	if(istype(M))
		...
		realvoice = name // IRIS ADDITION -- NTSL

...
/datum/quirk/item_quirk/breather/water_breather/add_unique(client/client_source)
	...
	for (var/datum/action/cooldown/slime_hydrophobia/hydrophobia_action in quirk_holder.actions) // OCULIS EDIT CHANGE - SLIMEPEOPLE - ORIGINAL: for (var/datum/action/cooldown/spell/slime_hydrophobia/hydrophobia_action in quirk_holder.actions)
...
/datum/quirk/item_quirk/breather/water_breather/proc/remove_hydrophobia_action(datum/action/action)
	if(QDELETED(quirk_holder))
		return
	if(istype(action, /datum/action/cooldown/slime_hydrophobia)) // OCULIS EDIT CHANGE - SLIMEPEOPLE - ORIGINAL: if(istype(action, /datum/action/cooldown/spell/slime_hydrophobia))
		qdel(action)

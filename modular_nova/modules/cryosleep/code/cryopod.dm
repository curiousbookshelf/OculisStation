...
/obj/machinery/cryopod/proc/despawn_occupant()
	...
	if(!HAS_TRAIT_FROM(mob_occupant, TRAIT_FREE_GHOST, TRAIT_GHOSTROLE)) // Don't let ghost cafe people store items
		for(var/obj/item/item_content in mob_occupant)
			if (!istype(item_content, /obj/item) || HAS_TRAIT(item_content, TRAIT_NODROP) || (item_content.item_flags & (ABSTRACT | DROPDEL)) || (item_content.flags_1 & HOLOGRAM_1) || (item_content.obj_flags_nova & NO_CRYO_FREEZE) || QDELETED(item_content)) //IRIS EDIT. ORIGINAL CODE: if(!istype(item_content) || HAS_TRAIT(item_content, TRAIT_NODROP) || (item_content.item_flags & ABSTRACT|DROPDEL) || (item_content.flags_1 & HOLOGRAM_1))
				continue
